...
})({"eNzjA":[function(require,module,exports) {
"use strict";
...
module.bundle.HMR_BUNDLE_ID = "beead87c7fc6ffdb";
...
function getTotalPrice() {
    const products = (0, _localStorageService.getProducts)();
    ...
    return productIds.reduce((acc, id)=>{
        ...
        const count = cart[id];
        return acc + product1.price.amount * count;
    }, 0);
}
...
function renderProducts() {
    ...
    const markup = productIds.map((id)=>{
        const product2 = products.find((product)=>product.id === id);
        const count = cart[id];
        const { name , price , img  } = product2;
        ...
        <div class="products-list__amount form-input">
          ...
          <input name="product" value="${count}" data-id="${product2.id}" class="form-input__input js-amount-iput" type="number" />
        </div>
        <div class="products-list__price">${price.amount} ${price.currency}</div>
        <a href="#" class="js-remove-from-cart products-list__remove-button" data-id="${product2.id}"></a>
      </li>`;
    }).join("");
...
function renderIfLastProductRemoved() {
    if (!Object.keys((0, _sessionStorageService.getCart)()).length) {
...
refs.cartList.addEventListener("click", (event)=>{
    if (event.target.classList.contains("js-remove-from-cart")) {
        (0, _cartService.removeFromCart)(event.target.dataset.id);
...
},{"./local-storage-service":"3Gm7A","./session-storage-service":"g2AEZ","./cart-service":"iinBt"}]},["eNzjA","lEDSO"], "lEDSO", "parcelRequired7c6")

//# sourceMappingURL=cart.7fc6ffdb.js.map
